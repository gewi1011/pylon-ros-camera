...
class image_bridge:
  ...
  def __init__(self):
    ...
    self.image_pub = rospy.Publisher("image_bridge_opencv_01/detected_circles_img",Image, queue_size=1) 
    self.flag_pub = rospy.Publisher("image_bridge_opencv_01/detected_circles_flag", Int8, queue_size=1)
    self.distance_pub = rospy.Publisher("image_bridge_opencv_01/distance",Int16, queue_size=1)
    

    self.bridge = CvBridge()
    self.image_sub = rospy.Subscriber("/camera_01/pylon_camera_node/image_rect",Image,self.callback)
  ...
  def callback(self,data):
    ...
    p1=cv2.getTrackbarPos("param1", "detected_circles_01")
    p2=cv2.getTrackbarPos("param2", "detected_circles_01")
     
    circles = cv2.HoughCircles(gray, cv2.HOUGH_GRADIENT, 1, 16, param1=p1, param2=p2, minRadius=1, maxRadius=100)
    ...
    if circles is not None:
	    ...
	    flag=1 #flag that 6 circles were detected
            rospy.loginfo('detection_flag: '+str(flag)) 
	    ...
	    rospy.loginfo('distance to camera :'+str(distance))
    ...
    cv2.imshow("detected_circles_01", cv_image)
...
def main(args):
  ...
  #creating a window
  cv2.namedWindow("detected_circles_01",cv2.WINDOW_NORMAL)
  cv2.resizeWindow("detected_circles_01",800,800)
  ...
  cv2.createTrackbar("param1","detected_circles_01",169,255,empty)  
  cv2.createTrackbar("param2","detected_circles_01",48,100,empty)	

  rospy.init_node('image_bridge_01', anonymous=True)
  ib = image_bridge() #creating object of the class "image_bridge"
  rospy.loginfo('image_bridge_01 node started')
